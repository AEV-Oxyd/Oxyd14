...
public sealed class SharedSkillSystemTests : ContentUnitTest
{
    ...
    [Test]
    public void SetUniqueBuff_UpdatesAmountAndRecalculates()
    {
        var entity = CreateSkillEntity();
        try
        {
            var original = _skillSystem.SetUniqueBuff(entity, "litany", 2, SkillId, null);
            var updated = _skillSystem.SetUniqueBuff(entity, "litany", 7, SkillId, null);

            using (Assert.EnterMultipleScope())
            {
                Assert.That(updated, Is.SameAs(original));
                Assert.That(updated, Is.SameAs(entity.Comp.buffSources[SkillId]["litany"][0]));
                Assert.That(updated.amount, Is.EqualTo(7));
                Assert.That(entity.Comp.skills[SkillId][1], Is.EqualTo(7));
    ...
    [Test]
    public void SetUniqueBuff_RefreshesExpirationAndExpires()
    {
        var entity = CreateSkillEntity();
        try
        {
            ...
            var refreshed = _skillSystem.SetUniqueBuff(entity, "litany", 4, SkillId, TimeSpan.FromSeconds(5));

            using (Assert.EnterMultipleScope())
            {
                Assert.That(refreshed, Is.SameAs(initial));
                Assert.That(refreshed.expires, Is.EqualTo(TimeSpan.FromSeconds(17)));
                Assert.That(entity.Comp.skills[SkillId][1], Is.EqualTo(4));
            }
