...
public sealed partial class LitanyWindow : FancyWindow
{
    ...
    private IEnumerable<LitanyViewerEntry> GetFilteredEntries()
    {
        ...
        return _snapshot.Entries
            .Where(entry => _categoryFilter is null || GetLitanyCategory(entry) == _categoryFilter)
            .Where(entry => string.IsNullOrEmpty(search)
                            || GetLitanyName(entry).Contains(search, StringComparison.CurrentCultureIgnoreCase)
                            || GetLitanyDescription(entry).Contains(search, StringComparison.CurrentCultureIgnoreCase))
            .OrderBy(entry => (int) GetLitanyCategory(entry))
            .ThenBy(GetLitanyName, StringComparer.CurrentCultureIgnoreCase)
    ...
    private void UpdateProfilePresentation()
    {
        ...
        HolinessBar.Value = double.IsFinite(snapshot.Holiness)
            ? (float) Math.Clamp(snapshot.Holiness, 0d, Math.Max(1d, maximumHoliness))
            : 0f;
    }
    ...
    private void UpdateBusyPresentation()
    {
        ...
        var hasCruciform = _hasRolePresentation
            ? _hasCruciform
            : _snapshot.Active || GetProfile(_snapshot.Profile) is not null;
    ...
    private bool CanBeginSelectedLitany()
    {
        ...
        if (!entry.Available
            ...
            return false;
        if (!double.IsFinite(snapshot.Holiness) || !double.IsFinite(litany.Cost) || litany.Cost < 0)
            return false;
    ...
    private bool RequiresChoice(LitanyViewerEntry entry)
    {
        return TryGetLitany(entry, out var litany) && litany.TargetMode is not LitanyTargetMode.Self
    ...
    {
        if (_prototypeManager.TryIndex(entry.Litany, out LitanyPrototype? prototype) && prototype != null)
        {
            litany = prototype;
            return true;
        }
    ...
    private string GetLitanyName(LitanyViewerEntry entry)
    {
    ...
    {
        return _prototypeManager.TryIndex(accessId, out AccessLevelPrototype? access)
            ? access.GetAccessLevelName()
            : accessId.ToString();
    }

    private NeoTheologyProfilePrototype? GetProfile(ProtoId<NeoTheologyProfilePrototype>? profileId)
    {
        if (profileId is { } id &&
            _prototypeManager.TryIndex(id, out NeoTheologyProfilePrototype? profile) &&
            profile is not null)
        {
            return profile;
        }
    ...
    private static double GetMaximumHoliness(NeoTheologyProfilePrototype? profile)
    {
        var capacity = profile?.CruciformCapacity ?? 0d;
        return double.IsFinite(capacity) && capacity >= 0 ? capacity : 0d;
    }
